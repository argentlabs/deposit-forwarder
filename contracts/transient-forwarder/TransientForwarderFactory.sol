...
import "./TransientForwarder.sol";

contract TransientForwarderFactory {

    address public implementation;
    bytes public initCode;

    constructor() {
        implementation = address(new TransientForwarder());
    ...
    function forward(address payable _wallet, address _token) external {
        TransientForwarder forwarder = _deployForwarder(_wallet);
        forwarder.forwardAndDestruct(_wallet, _token);
    }

    function _deployForwarder(address _wallet) internal returns (TransientForwarder forwarder) {
